...
export function ImageCleanupPage({
  initialMode = "watermark_remove",
  isAuthenticated,
  onRequireLogin,
  onOpenPricing,
  initialProduct,
  onInitialProductConsumed,
}: ImageCleanupPageProps) {
  ...
  const handleGenerate = async () => {
    ...
    if (!product) {
      setNotice({
        title: "请先选择图片",
        message: "从图片库选择需要处理的图片后，再涂抹清理区域。",
      });
      return;
    }
    ...
    if (!maskBase64) {
      setNotice({
        title: "还没有涂抹区域",
        message: "请先用红色画笔覆盖需要去除的水印、文字或物体。",
      });
      return;
    }
    ...
    void saveGenerationTasks([
      processingTask,
      ...getGenerationTaskSnapshot().filter(
        (task) => task.id !== processingTask.id,
      ),
    ]);
    ...
    try {
      ...
      const result = await runImageCleanup({
        ...
        onTaskStarted: (backendTaskId) => {
          ...
          void saveGenerationTasks([
            ...
            ...getGenerationTaskSnapshot().filter(
              (task) => task.id !== currentProcessingTask.id,
            ),
          ]);
        },
      });
      const completed = completeTask(currentProcessingTask, {
        ...
        resultAssets: [
          {
            url: result.imageUrl,
            label: mode === "watermark_remove" ? "去除水印或文字" : "移除物体",
            ...(result.channelUsed ? { channelUsed: result.channelUsed } : {}),
          },
        ],
        ...(result.channelUsed
          ? {
              channelUsed: result.channelUsed,
              channelUsedByAsset: [result.channelUsed],
            }
          : {}),
        creditCost: 1,
      ...
      try {
        ...
        setStatus(
          "清理完成并已保存结果，但积分同步暂时失败；结果不会被标记为失败。",
        );
      }
    } catch (error) {
      const failed = failTask(currentProcessingTask, {
        errorCode: "image_cleanup_failed",
        errorMessage: error instanceof Error ? error.message : "图片清理失败",
        completedAt: new Date().toISOString(),
      });
      await saveGenerationTaskHistory(failed);
      await saveGenerationTasks([
        failed,
        ...getGenerationTaskSnapshot().filter((task) => task.id !== failed.id),
      ]);
  ...
  return (
    <main className="cleanup-page page-surface">
      <section className="page-heading">
        <p className="eyebrow">MASKED IMAGE CLEANUP</p>
        <h1>图片清理</h1>
        <p>
          涂抹水印、文字、标记或不需要的物体，只修改画笔覆盖区域。仅处理你拥有或获授权的图片。
        </p>
      </section>
      <div className="cleanup-workbench">
        <section className="panel cleanup-settings" aria-label="图片清理设置">
          <button
            type="button"
            className="cleanup-upload"
            aria-label="从图片库选择待清理图片"
            onClick={() => setPickerOpen(true)}
          >
            <Brush aria-hidden="true" />
            <span>从图片库选择待清理图片</span>
            <small>本地图片请先到图片库批量上传</small>
          </button>
          <div className="segmented-control" aria-label="清理类型">
            <button
              type="button"
              className={mode === "watermark_remove" ? "is-active" : undefined}
              aria-pressed={mode === "watermark_remove"}
              onClick={() => setMode("watermark_remove")}
            >
              去除水印或文字
            </button>
            <button
              type="button"
              className={mode === "remove_object" ? "is-active" : undefined}
              aria-pressed={mode === "remove_object"}
              onClick={() => setMode("remove_object")}
            >
              移除物体
            </button>
          </div>
          <label className="cleanup-brush-size">
            <span>画笔大小</span>
            <input
              type="range"
              min={12}
          ...
          <button
            type="button"
            className="primary-button cleanup-generate"
            disabled={!product || isGenerating}
            onClick={handleGenerate}
          >
            <Sparkles aria-hidden="true" />
            {isGenerating ? "正在清理" : "开始清理（1 积分）"}
          </button>
          <p className="cleanup-status" role="status">
            {status}
          </p>
        </section>
          ...
          {product ? (
            <div className="cleanup-canvas-wrap">
              <img
                ...
                onError={() => {
                  const errorMessage = "图片加载失败，请返回图片库重新选择。";
                  setStatus(errorMessage);
                  setNotice({ title: "图片加载失败", message: errorMessage });
                }}
              />
              <canvas
                ref={canvasRef}
                ...
                onPointerMove={(event) => {
                  ...
                  drawingRef.current.points.push(pointerPoint(event));
                  redrawMask();
                }}
                onPointerUp={() => {
            ...
            <div className="cleanup-empty">
              <Brush aria-hidden="true" />
              <strong>等待图片</strong>
              <span>从图片库选择后，用红色画笔涂抹清理区域</span>
            </div>
          )}
          {resultUrl ? (
            <div className="cleanup-result">
              <img src={resultUrl} alt="图片清理结果" />
              <button
                type="button"
                className="primary-button"
                onClick={() =>
                  void downloadCleanupResult(resultUrl).catch((error) => {
                    const errorMessage =
                      error instanceof Error ? error.message : "下载失败";
                    setStatus(errorMessage);
                    setNotice({ title: "结果下载失败", message: errorMessage });
                  })
                }
              >

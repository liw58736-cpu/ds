...
const featureModules: FeatureModule[] = [
  {
    ...
    title: "商品主图和首屏 KV",
    summary:
      "把普通商品照重做成第一眼能卖货的主视觉，适合上架、投放和独立站首屏。",
    beforeLabel: "卖家原始图",
    afterLabel: "kroma 主图",
  ...
  },
  {
    ...
    title: "白底图和平台抠图",
    summary:
      "保留商品结构和材质，整理成平台审核友好的白底图、目录图和 SKU 图。",
    beforeLabel: "场景商品图",
    afterLabel: "白底成片",
    ...
    <main className="home-page kroma-home">
      <section className="kroma-home-hero" aria-labelledby="home-title">
        <div className="kroma-home-hero-copy">
          <p className="kroma-home-eyebrow">
            <Sparkles aria-hidden="true" /> KROMA · AI COMMERCE STUDIO
          </p>
        ...
        <div
          className="kroma-home-hero-stage"
          aria-label="kroma 商品图生成展示"
        >
      ...
      <section className="kroma-home-showcase" aria-labelledby="showcase-title">
        ...
        <div className="kroma-home-showcase-layout">
          <div
            className="kroma-home-showcase-tabs"
            role="tablist"
            aria-label="kroma 核心功能"
          >
            {featureModules.map((module, index) => (
              <button
                ...
                className={
                  module.page === activeShowcase ? "is-active" : undefined
                }
                key={module.page}
          ...
          <div className="kroma-home-showcase-panels">
            {featureModules.map((module) => (
              ...
              >
                ...
                <div
                  className="kroma-home-before-after"
                  aria-label={`${module.title}前后对比`}
                >
                  <figure>
                    <span>{module.beforeLabel}</span>
                    <img
                      src={module.beforeImage}
                      alt={`${module.title}出图前`}
                    />
                  </figure>
                  <figure className="is-after">
                    <span>{module.afterLabel}</span>
                    <img
                      src={module.afterImage}
                      alt={`${module.title}出图后`}
                    />
                  </figure>
                </div>
      ...
      <section
        className="kroma-home-content-tools"
      ...
      >
        ...
        <div className="kroma-home-content-grid">
          <article>
            ...
            <span>01</span>
            <h3>链接提取与图片库</h3>
            <p>粘贴小红书分享文案，多选需要的照片并保存到个人图片库。</p>
            <button type="button" onClick={() => onOpenStudio("materials")}>
              打开图片库
              <ArrowRight aria-hidden="true" />
            </button>
          </article>
      ...
      <section className="kroma-home-output" aria-labelledby="output-title">
        ...
        <div className="kroma-home-output-copy">
          ...
          <p>
            主图、白底图、详情模块和历史任务保持连续，让上新素材能够按同一套标准持续产出。
          </p>
          <div className="kroma-home-output-facts">
            <span>
              <strong>1K / 2K / 4K</strong>多分辨率输出
            </span>
            <span>
              <strong>失败不扣点</strong>任务结果可追溯
            </span>
          </div>
        </div>
      </section>

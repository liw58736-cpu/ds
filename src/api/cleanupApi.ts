...
function normalizeResult(task: CleanupTaskResponse): CleanupResult {
  ...
  return {
    imageUrl,
    ...(task.billing_managed ? { billingManaged: true } : {}),
...
export async function runImageCleanup(input: {
  ...
  const label =
    input.mode === "watermark_remove" ? "水印、文字或标记" : "不需要的物体";
  const response = await fetchWithAuthRefresh(`${baseUrl}/image/generate`, {
    ...
    body: JSON.stringify({
      ...(input.task
        ? {
            context: {
              requestId: `${input.task.id}:${input.task.attempt}:0`,
              groupId: input.task.id,
              index: 0,
              total: 1,
              product: input.task.productInput,
              config: input.task.config,
              childConfig: input.task.config,
              label:
                input.mode === "watermark_remove" ? "局部清理" : "移除物体",
      ...
      prompt:
        input.prompt?.trim() ||
        `仅移除画笔标记范围内的${label}，自然修复背景，未标记区域保持完全不变。`,
  ...
  if (!response.ok)
    throw new Error(
      `图片清理提交失败（HTTP ${response.status}）：${await response.text()}`,
    );
  ...
  for (let attempt = 0; attempt < 360; attempt += 1) {
    ...
    if (task.status === "error")
      throw new Error(task.error || "图片清理失败，请重试。");
    input.onProgress?.("正在清理标记区域");
    await new Promise((resolve) => setTimeout(resolve, 2000));
    const poll = await fetchWithAuthRefresh(
      `${baseUrl}/image/task/${encodeURIComponent(task.task_id)}`,
      { method: "GET" },
    );
    if (!poll.ok)
      throw new Error(`图片清理任务查询失败（HTTP ${poll.status}）`);

...
async function navigateStudio(page: Page, name: string) {
...
async function generateSampleAsset(page: Page) {
  ...
  await page.getByRole("button", { name: /首屏 KV 建立/ }).click();
...
async function seedImageLibrary(page: Page) {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "kroma-tasks-v2:account%3Aseller%40example.com",
      JSON.stringify([
        createTask(
          "library-2",
          "library-detail.webp",
          "/src/assets/home/kroma-detail-before-v2.webp",
          "2026-09-03T00:01:00.000Z",
        ),
        createTask(
          "library-1",
          "library-main.webp",
          "/src/assets/home/kroma-main-before-v2.webp",
...
async function chooseImageLibraryAsset(
  page: Page,
  triggerName: string,
  fileName: string,
) {
...
test("workspace uploads stay isolated by navigation and detail titles remain horizontal", async ({
  page,
}) => {
  await seedAuthenticatedAccount(page);
  ...
  const firstModuleTitle = page
    .locator(".detail-module-button .module-card-topline strong")
    .first();
  await expect(firstModuleTitle).toBeVisible();
  await expect
    .poll(() =>
      firstModuleTitle.evaluate((element) => ({
...
test("detail page quantity controls create multiple results for the same module", async ({
  ...
  await firstDetailModule
    .locator(".detail-module-stepper button")
    .last()
    .click();
  await expect(
    firstDetailModule.locator(".detail-module-stepper b"),
  ).toHaveText("2");
  await expect(page.locator(".generation-footer")).toContainText("2");
...
test("navigation surfaces render and preserve generated history", async ({
  ...
  await expect(
    page.getByRole("button", { name: "一次性购买" }),
  ).toHaveAttribute("aria-pressed", "true");
  const payButtons = page.getByRole("button", { name: "购买积分" });
  ...
  await expect(page.locator(".topnav-button")).toHaveCount(6);
  ...
  await expect(
    page.getByRole("heading", { name: "任务中心", level: 1 }),
  ).toBeVisible();
  await expect(
    page.getByRole("region", { name: "历史任务统计" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "最近任务", level: 2 }),
  ).toBeVisible();
  await expect(page.getByAltText("当前商品图")).toBeHidden();
  await expect(
    page
      .locator(".history-result-grid")
      .getByAltText("生成结果缩略图", { exact: true }),
  ).toBeVisible();
  await expect(
...
test("new content tools render and remain usable without horizontal overflow", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Live 图生成" }),
  ).toBeVisible();
  await expect(
  ...
  await expect(page.getByLabel("从本地批量上传图片")).toHaveAttribute(
    "multiple",
    "",
  );
  await page.getByRole("button", { name: "链接提取", exact: true }).click();
  await expect(page.getByRole("heading", { name: "链接提取" })).toBeVisible();
  await expectNoHorizontalDocumentOverflow(page);
});

test("guest Xiaohongshu extraction uses a visible login dialog", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: "图片库", exact: true }).click();
...
test("two-image inspiration results compare original and generated images and can open Live creation", async ({
  ...
  await expect(
    page.getByRole("button", { name: "放大查看 灵感创作" }),
  ).toBeVisible();
...
test("full drafts survive home navigation and browser reload without crossing tools", async ({
  page,
}) => {
  await seedAuthenticatedAccount(page);
  await seedImageLibrary(page);
  ...
  await page.getByLabel("设计简报").fill("主图专用文案");
  await page.getByLabel("输出语言").selectOption("English");
  await navigateStudio(page, "详情页");
  await expect(page.getByLabel("组图要求")).toHaveValue("");
  await page.getByLabel("组图要求").fill("详情专用文案");
  await page.getByRole("button", { name: "首页", exact: true }).click();
  await navigateStudio(page, "商品主图");
  await expect(page.getByLabel("设计简报")).toHaveValue("主图专用文案");
  await page.reload();
  await navigateStudio(page, "商品主图");
  await expect(page.getByLabel("设计简报")).toHaveValue("主图专用文案");
  await expect(page.getByLabel("输出语言")).toHaveValue("English");
  await navigateStudio(page, "详情页");
  await expect(page.getByLabel("组图要求")).toHaveValue("详情专用文案");
});

test("library upload detour returns to the same picker and preserves its image role", async ({
  page,
}) => {
  await seedAuthenticatedAccount(page);
  await seedImageLibrary(page);
  ...
  await expect(page.getByAltText("替换产品服装图")).toBeVisible();
  await expect(page.getByAltText("灵感原图", { exact: true })).toHaveCount(0);
});

test("local upload stays exclusive to library and cleanup stays inside AI tools", async ({
  page,
}) => {
  await seedAuthenticatedAccount(page);
  await page.goto("/");
  await navigateStudio(page, "AI工具");
  await useSampleProductByCss(page);
  await page.getByRole("button", { name: "局部清理", exact: true }).click();
  await expect(
    page.getByRole("heading", { name: "图片清理", exact: true }),
  ).toBeVisible();
  await expect(page.locator("input[type=file]")).toHaveCount(0);
